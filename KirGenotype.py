...
def GetGenotype(geneAbund):
	...
	for gene in majorGeneAbund:
		...
		for allele in allelesInGene[gene]:
			if (majorAlleleAbund[allele][1] >= majorGeneAbund[gene][1] * 0.1 and majorAlleleAbund[allele][0] >= 2):
				l.append([allele, majorAlleleAbund[allele][1]])
		l = sorted(l, key = lambda x: x[1], reverse=True)
		genotype[gene] = []
		for i in range(0, min(2, len(l))):
			genotype[gene].append(l[i][0])
	return genotype, majorGeneAbund
...
if (__name__ == "__main__"):
	...
	genotype, majorGeneAbund = GetGenotype(geneAbund)
	for gene in sorted(list(genotype.keys())):
		print(gene + "\t" + "\t".join([str(x) for x in majorGeneAbund[gene]]) + "\t" + "\t".join(genotype[gene])) 
